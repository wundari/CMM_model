...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 19:46:41 2021
...
import Lib_SVM_PermuteDecode as svm_std

#sys.path.append("../MVPA")
#from MVPA import MVPA_Decode as svm_std
...
class RSA(General):
    ...
    def _compute_rdm_dist(self,
        ...
        mtd : string, optional
            method for computing dissimilarity. The default is "correlation".
        ...
        # sbjID = rsa.sbjID_all[sbj]
        # nRuns = rsa.nRuns_all[sbj]
        # P_data = rsa.load_P_data(sbjID, nRuns)

        # process P_data
        P_data = self.label_P_data(P_data)
        # P_data = rsa.label_P_data(P_data)

        # normalize P_data
        P_data = self.normalize_P_data(P_data)  # fixation is excluded here
        # P_data = rsa.normalize_P_data(P_data)

        conds = P_data.cond.unique()  # there is no fixation cond here
        n_conds = len(conds)  # number of conditions
        nRuns = len(P_data.run.unique())  # number of scan runs
        ...
        for roi in range(len(self.ROIs)):
            ...
            P_roi = P_data.loc[(P_data.roi == roi+1) &
            ...
            for cond1 in range(n_conds):  # loop excluding fixation
                for cond2 in range(cond1+1, n_conds):
                    ...
                    dist = cdist(x_mean, y_mean, mtd)  # [[1]]
                    rdm_roi[cond1, cond2] = c*np.mean(dist)
                    # rdm_roi[cond1, cond2] = c*(2 - np.mean(dist))
    ...
    def compute_rdm_all_sbjID(self,
        ...
        Returns
        -------
        rdm_all : [n_sbjID, n_ROIs, 6, 6] np.array
            representational dissimilarity matrix

        '''

        rdm_all = []
        rdm_all.append(Parallel(n_jobs=-1)
                       (delayed(self._compute_rdm_dist)
                       (sbj, nVox_to_analyze, mtd)
                       for sbj in range(self.n_sbjID)))

        # unpack
        rdm_fmri_all = np.zeros((self.n_sbjID, self.n_ROIs, 6, 6),
                                dtype=np.float32)

        for sbj in range(self.n_sbjID):

            rdm_fmri_all[sbj] = rdm_all[0][sbj]

        self.rdm_fmri_all = rdm_fmri_all
    ...
    def compute_diss_fmri_cmm(self, CMM,
        ...
        # [len(f_batch), 6, 6]
        rdm_corr_avg = np.mean(rdm_corr_bootsrap, axis=1)
        # [len(f_batch), 6, 6]
        rdm_match_avg = np.mean(rdm_match_bootsrap, axis=1)
            ...
            for i_f in range(len(CMM.f_batch)):
                ...
                for sbj in range(len(self.rdm_fmri_all)):
                    ...
                    for i in range(6):
                        ...
                        diss_corr[roi, i_f, sbj, i] = 0.5 * \
                            (1 - stats.pearsonr(x, y)[0])
    ...
    def _compute_kendalltau_up_single_roi(self, rdm_fmri_all,
        ...
        for i in range(n_bootstrap):
            ...
            id_sample = np.random.choice(
                sbjID_bootsrap, size=len(sbjID_bootsrap))
            ...
            kendalltau_up_roi[i] = kendalltau(
                rdm_fmri_above, rdm_boot_above)[0]
    ...
    def _compute_kendalltau_low_single_roi(self, rdm_fmri_all,
        ...
        for i in range(n_bootstrap):
            ...
            kendalltau_low_roi[i] = kendalltau(
                rdm_fmri_above, rdm_boot_above)[0]
    ...
    def compute_noiseCeiling(self, rdm_fmri_all,
        ...
        kendalltau_low = np.zeros(
            (n_roi, n_bootstrap, n_sbj), dtype=np.float32)
    ...
    def compute_rdm_svm(self, useVoxVal,
        ...
        for i in range(6):
            for j in range(i+1, 6):
                ...
                for roi in range(len(self.ROIs)):
                    rdm_svm[roi, :, i,
                            j] = score_avg.loc[score_avg.roi == roi+1].acc_avg
