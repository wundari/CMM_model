...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  1 12:58:27 2021
...
class RSA(General):
    ...
    def _compute_rdm_dist(self, vtc_normalized,
                           sbj,
                           nVox_to_analyze,
                           mtd="correlation"):
        '''
        ...
        conds =  vtc_normalized.cond.unique() # there is no fixation cond here
        ...
        rdm_all_roi = np.zeros((self.n_ROIs, n_conds, n_conds))
        # rdm_all_roi = np.zeros((rsa.n_ROIs, n_conds, n_conds))
        ...
        for roi in range(self.n_ROIs):
            ...
            vtc_roi = vtc_normalized.loc[(vtc_normalized.roi==roi) &
                                         (vtc_normalized.vox.isin(range(nVox_to_analyze)))]
            
            # average vtc_norm across rep
            vtc_roi_cond = (vtc_roi.groupby(["cond", "vox", "run"])
                                   .vtc_norm
                                   .agg(["mean"])
                                   .reset_index())
            vtc_roi_cond = vtc_roi_cond.rename(columns={"mean":"avg"})
            
            # collect data for each condition and run
            df = vtc_roi_cond.pivot_table(index="run",
                                          columns=["cond", "vox"],
                                          values="avg")
    ...
    def compute_rdm_all_sbjID(self,
        ...
        # mtd = "correlation"
        rdm_all = np.zeros((self.n_sbjID, self.n_ROIs, 6, 6))
        ...
        for sbj in range(self.n_sbjID):
            ...
            # # vtc_labeled = rsa.load_vtc_labeled(sbjID)
            
            # # normalize with time-shifted 2TR to account for hrf delay.
        
            # ## z-score fmri time-series for each roi, run, vox
            # avg = (vtc_labeled.groupby(["roi", "run", "vox"])
            #                    .vtc_value
            #                    .transform("mean"))
            # std = (vtc_labeled.groupby(["roi", "run", "vox"])
            #                    .vtc_value
            #                    .transform(np.std))
            ...
            # # t_start = timer()
            # vtc_roi_all = []
            # vtc_roi_all.append(Parallel(n_jobs=self.n_ROIs)
            #                    (delayed(self.shift_vtc_roi)
            #                     (vtc_normalized, t_stat_sbj,
            #                      roi) for roi in range(self.n_ROIs)))
            # # vtc_roi_all.append(Parallel(n_jobs=rsa.n_ROIs, backend="threading")
            # #                    (delayed(rsa.shift_vtc_roi)
            # #                     (vtc_normalized, t_stat_sbj,
            # #                      roi) for roi in range(rsa.n_ROIs)))
            ...
            # vtc_normalized = (vtc_roi_df.groupby(["roi", "vox", "stimID", "cond", "run", "rep"])
            #                       .vtc_norm
            #                       .agg(["mean"])
            #                       .reset_index())
            # vtc_normalized = vtc_normalized.rename(columns={"mean":"vtc_norm"})
            
            sbjID = self.sbjID_all[sbj]
            vtc_normalized = self.load_vtc_normalized_vox(sbjID, nVox_to_analyze)
            # vtc_normalized = rsa.load_vtc_normalized_vox(sbjID, nVox_to_analyze)
            # compute rdm and store it according to partiality
            rdm_allROI = self._compute_rdm_dist(vtc_normalized,
                                                sbj,
                                                nVox_to_analyze,
                                                mtd)
