...
class RDS(General):
    ...
    def load_rds_disp_tuning(self, rds_type):
        ...
        Returns:
            rds dimension:
            <[n_rds_trial, len(disp_ct_deg), size_rds, size_rds] np.array>
        ...
        print("load rds: {}".format(rds_type))
        self.L = np.zeros(
            (self.n_rds_trial, 21, self.size_rds_bg_pix, self.size_rds_bg_pix),
            dtype=np.int8,
        )
        self.R = np.zeros(
            (self.n_rds_trial, 21, self.size_rds_bg_pix, self.size_rds_bg_pix),
        ...
        if rds_type == "ards":
            ...
            self.L[0 : self.n_rds_trial] = temp[0 : self.n_rds_trial]

            temp = np.load("../../../Data/rds_small_pixel/disp_tuning/rds_right_a.npy")
            self.R[0 : self.n_rds_trial] = temp[0 : self.n_rds_trial]
        ...
        self.u_L[0 : self.n_rds_trial] = temp[0 : self.n_rds_trial]

        temp = np.load("../../../Data/rds_small_pixel/disp_tuning/rds_right_u.npy")
        self.u_R[0 : self.n_rds_trial] = temp[0 : self.n_rds_trial]
    ...
    def load_rds_multivariate_analysis(self, rds_type):
        ...
        """
        print("load rds: {}".format(rds_type))
        # self.L = np.zeros((self.n_rds_trial, 4,
        #                    self.size_rds_bg_pix, self.size_rds_bg_pix),
        #                   dtype=np.int8)
        # self.R = np.zeros((self.n_rds_trial, 4,
        #                    self.size_rds_bg_pix, self.size_rds_bg_pix),
        #                   dtype=np.int8)
        ...
        if rds_type == "ards":
            ...
            self.L = temp[0 : self.n_rds_trial]

            temp = np.load(
                "../../../Data/rds_small_pixel/multivariate_analysis/rds_right_a.npy"
            )
            self.R = temp[0 : self.n_rds_trial]
        ...
        self.u_L = temp[0 : self.n_rds_trial]

        temp = np.load(
            "../../../Data/rds_small_pixel/multivariate_analysis/rds_right_u.npy"
        )
        self.u_R = temp[0 : self.n_rds_trial]
