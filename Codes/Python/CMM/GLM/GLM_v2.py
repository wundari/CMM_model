...
class GLM(General):
    ...
    def diagnose_glm(self, beta_all_sbjID, sbj, roi, vox, run, mtd_normalization):
        ...
        # beta = beta_avg_roi_all_sbjID[sbj, roi, 0:p]
        ...
        # # filter dataframe according to roi and nVox
        # vtc_roi = vtc_labeled.loc[(vtc_labeled.roi==roi+1)]

        # # mtd_normalization = 1 # % bold change
        # y = glm.normalize_vtc_roi(vtc_roi, mtd_normalization)[:, 0:nVox].mean(axis=1)
        ...
        # plt.figure(figsize=(10, 10))
        # # vox = 0
        # i_start = run*glm.n_timepoints_per_run
        # i_end = i_start + glm.n_timepoints_per_run

        # plt.plot(y[i_start:i_end]) # plot y_true
        # plt.plot(y_glm[i_start:i_end]) # plot y_glm
        # plt.plot(e[i_start:i_end])
        # plt.legend(["y_true", "y_glm", "error"])

        # plt.xlabel("Timepoint")
        # if mtd_normalization==0:
        #     plt.ylabel("z-score")
        # elif mtd_normalization==1:
        #     plt.ylabel("% BOLD-change")
        # elif mtd_normalization==2:
        ...
        # plt.title("GLM diagnostic\nsbjID: {}, ROI: {}, vox: {}"
        #           .format(self.sbjID_all[sbj], self.ROIs[roi], str(vox+1)))
