...
class RDS:
    def __init__(self, n_rds, w_bg, h_bg, w_ct, h_ct, dotDens, rDot, overlap_flag):
    ...
    def create_rds(self, disp_ct_pix, dotMatch_ct):
        ...
        for i in range(rdsDisp_channels):
            ...
            rds_ct_left, rds_ct_right = self._set_dotMatch(
                rds_ct, dotMatch_ct, nDots, self.rDot
            )

            ## put rds_ct into rds_bg
            # horizontal disparity using the left disparity map:
            # disp_ct_pix < 0 -> (crossed-disparity) near:
            #                       put the dots in RDS_right to the left RDS_left
            # disp_ct_pix > 0 -> (uncrossed-disparity) far:
            #                       put the dots in RDS_right to the right RDS_left
            ...
            col_start = center[1] - rds_ct_left.shape[1] // 2
            ...
            col_start = center[1] - rds_ct_right.shape[1] // 2 + disp_ct_pix[i]
