...
from SVM.svm_analysis_v2 import *
...
class RDSAnalysis(Engine):
    ...
    def compute_disp_map_rds(self, dotMatch, dotDens, background_flag, pedestal_flag):
        ...
        print(f"disp map RDS dotMatch: {dotMatch:.1f}, dotDens: {dotDens:.1f}")
        ...
        for i, (inputs_left, inputs_right, disps) in enumerate(tepoch):
            ...
            # logits = self.model(input_left, input_right)  # [n_batch, maxdisp, h, w]
            # pred = torch.sum(logits.mul(self.disp_indices), 1)  # [n_batch, h, w]

            # model output
            pred = self.predict(input_left, input_right)  # [n_batch, h, w]

            id_start = i * self.batch_size
            id_end = id_start + self.batch_size
            pred_disp_labels[id_start:id_end] = disps
            pred_disp[id_start:id_end] = pred

            tepoch.set_description(
                f"RDS dotMatch: {dotMatch:.1f}, "
                + f"dotDens: {dotDens:.1f}, "
    ...
    def plotLine_xDecode_at_dotDens(self, dotDens, save_flag):
        ...
        fig.text(
            0.5,
            1.0,
            f"SVM prediction on RDS with background at dotDens {dotDens:.1f}",
            ha="center",
        )
